...
public class CargoServiceImpl implements CargoService {
    ...
    @Override
    @Transactional
    public CargoVO update(String cargoId, CargoUpdateDTO updateDTO) {
        String safeCargoId = trimToNull(cargoId);
        if (!StringUtils.hasText(safeCargoId)) {
            throw new IllegalArgumentException("货物 ID 不能为空");
        }
        ...
        CargoRecord current = findRequiredByCargoId(safeCargoId);
        if ("DELIVERED".equals(current.getStatus()) || "CANCELLED".equals(current.getStatus())) {
            throw new IllegalArgumentException("已送达或已取消的货物不能修改");
        }
        validateWeight(updateDTO.getWeight());
        ...
        cargo.setOriginLat(current.getOriginLat());
        cargo.setOriginLng(current.getOriginLng());
        ...
        fillLocation(cargo, updateDTO.getOrigin(), true);
        fillLocation(cargo, updateDTO.getDestination(), false);

        cargoMapper.update(cargo);
        return detail(safeCargoId);
    }

    @Override
    @Transactional
    public CargoVO cancel(String cargoId) {
        String safeCargoId = trimToNull(cargoId);
        if (!StringUtils.hasText(safeCargoId)) {
            throw new IllegalArgumentException("货物 ID 不能为空");
        }
        ...
        if ("DELIVERED".equals(current.getStatus())) {
            throw new IllegalArgumentException("已送达的货物不能取消");
        }
        if ("CANCELLED".equals(current.getStatus())) {
            return detail(safeCargoId);
        }

        CargoVehicleBinding binding = cargoMapper.findActiveBindingByCargoId(safeCargoId);
        if (binding != null) {
            cargoMapper.unbindById(binding.getId());
        }

        cargoMapper.updateCargoStatus(safeCargoId, "CANCELLED");
        cargoMapper.insertStatusLog(buildStatusLog(safeCargoId, "CANCELLED", null));
        return detail(safeCargoId);
    }
    ...
    public CargoVO bind(CargoBindDTO bindDTO) {
        ...
        if ("DELIVERED".equals(cargo.getStatus()) || "CANCELLED".equals(cargo.getStatus())) {
            throw new IllegalArgumentException("已送达或已取消的货物不能绑定车辆");
        }
        ...
        Long activeBindingCount = cargoMapper.countActiveBindingByCargoId(cargoId);
        ...
        cargoMapper.insertStatusLog(buildStatusLog(cargoId, "LOADED", null));
    ...
    public CargoVO unbind(CargoUnbindDTO unbindDTO) {
        ...
        cargoMapper.insertStatusLog(buildStatusLog(cargoId, "CREATED", null));
        return detail(cargoId);
    }
    ...
    public CargoVO updateStatus(String CargoID,CargoStatusUpdateDTO updateDTO) {
        ...
        cargoMapper.insertStatusLog(buildStatusLog(SafeID, Status, updateDTO));
    ...
    private CargoRecord findRequiredByCargoId(String cargoId) {
        CargoRecord cargo = cargoMapper.findByCargoId(cargoId);
        if (cargo == null) {
            throw new IllegalArgumentException("货物不存在");
        }
    ...
    private CargoStatusLog buildStatusLog(String cargoId, String status, CargoStatusUpdateDTO updateDTO) {
        return CargoStatusLog.builder()
                .id("LOG-" + UUID.randomUUID().toString().replace("-", "").substring(0, 12).toUpperCase())
                .cargoId(cargoId)
                .status(status)
                .lat(updateDTO == null ? null : updateDTO.getLat())
                .lng(updateDTO == null ? null : updateDTO.getLng())
                .remark(updateDTO == null ? null : trimToNull(updateDTO.getRemark()))
                .operatorId(updateDTO == null ? null : trimToNull(updateDTO.getOperatorId()))
                .build();
    }

...
public class GpsKafkaConsumer {
    ...
    private void updateRedisLatest(String plate, Long vehicleId, GpsData gps, Instant ts) {
        try {
            ...
            pushData.put("plate", plate);
            pushData.put("vehicleId", vehicleId);
            pushData.put("lat", gps.getLat());
            pushData.put("lng", gps.getLng());
            pushData.put("speed", gps.getSpeed() == null ? 0 : gps.getSpeed());
            pushData.put("heading", gps.getHeading() == null ? 0 : gps.getHeading());
            if (gps.getAccuracy() != null) {
                pushData.put("accuracy", gps.getAccuracy());
            }
            pushData.put("timestamp", ts.toString());
            pushData.put("updatedAt", ts.toString());
            com.sky.logistics.controller.LogisticsWebSocketServer.broadcast("vehicle.position", pushData);

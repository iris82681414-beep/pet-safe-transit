...
public class VehicleServiceImpl implements VehicleService {
    ...
    @Override
    public VehicleActiveTaskVO activeTask(String plate) {
        String safePlate = trimToNull(plate);
        if (!StringUtils.hasText(safePlate)) {
            throw new IllegalArgumentException("车牌号不能为空");
        }

        Vehicle vehicle = vehicleMapper.findByPlate(safePlate);
        if (vehicle == null) {
            throw new IllegalArgumentException("车辆不存在");
        }

        List<VehicleActiveCargoVO> cargos = vehicleMapper.findActiveCargosByPlate(vehicle.getPlate());
        if (cargos == null) {
            cargos = Collections.emptyList();
        }

        return VehicleActiveTaskVO.builder()

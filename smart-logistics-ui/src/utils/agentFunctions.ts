...
export const agentFunctionDefinitions: AgentFunctionDefinition[] = [
  { name: 'navigate_page', description: '打开智慧物流系统页面', mutating: false, parameters: { page: '目标页面路由名称' } },
  ...
  { name: 'delete_vehicle', description: '删除车辆并解除货物绑定', mutating: true, parameters: { plate: '车牌号' } },
  { name: 'create_cargo', description: '新增货物或运单', mutating: true, parameters: { cargoId: '货物编号', name: '货物名称', category: '品类', origin: '起点', destination: '终点', weight: '重量' } },
  { name: 'bind_cargo_vehicle', description: '把货物调度并绑定到车辆', mutating: true, parameters: { cargoId: '货物编号', plate: '车牌号' } },
  { name: 'unbind_cargo_vehicle', description: '解除货物与车辆的绑定', mutating: true, parameters: { cargoId: '货物编号' } },
  { name: 'update_cargo_status', description: '更新货物状态', mutating: true, parameters: { cargoId: '货物编号', status: 'CREATED/LOADED/IN_TRANSIT/DELIVERED/CANCELLED', remark: '备注' } },
  { name: 'confirm_receipt', description: '确认货物收货', mutating: true, parameters: { cargoId: '货物编号' } },
  { name: 'submit_address_change', description: '为运单提交收货地址变更申请', mutating: true, parameters: { cargoId: '货物编号', address: '新地址', city: '城市', lat: '纬度', lng: '经度', reason: '改址原因' } },
  { name: 'confirm_unload_address', description: '确认运单卸货地址', mutating: true, parameters: { cargoId: '货物编号', address: '卸货地址', lat: '纬度', lng: '经度' } },
  { name: 'report_unload_abnormal', description: '上报卸货地址异常', mutating: true, parameters: { cargoId: '货物编号', reason: '异常原因', currentAddress: '当前位置' } },
  { name: 'rate_driver', description: '为已送达运单评价司机', mutating: true, parameters: { cargoId: '货物编号', score: '1至5分', comment: '评价内容' } },
  ...
  { name: 'query_cargo', description: '查询货物详情', mutating: false, parameters: { cargoId: '货物编号' } },
...
function cargoStatus(value: unknown): Cargo['status'] {
  ...
  if (!result) throw new Error(`不支持的货物状态 ${String(value || '')}`)
  return result
}
...
export async function callAgentFunction(name: unknown, args: AgentArguments = {}): Promise<AgentFunctionResult> {
  ...
  switch (functionName) {
    ...
    case 'create_cargo': {
      const id = required(args.cargoId || args.orderId || args.id, '货物编号')
      if (store.cargo.some((item) => item.id.toUpperCase() === id.toUpperCase())) throw new Error(`货物 ${id} 已存在`)
      const item: Cargo = {
        id, name: String(args.name || args.cargoName || '新货物'), category: String(args.category || args.cargoType || '普通货物'),
        origin: String(args.origin || '重庆仓库'), destination: String(args.destination || '待指定'),
      ...
      return { message: `货物 ${id} 已新增`, data: item, targetPage: 'warehouse' }
    }
    case 'bind_cargo_vehicle': {
      ...
      return { message: `已将货物 ${item.id} 调度到车辆 ${vehicle.plate}`, targetPage: 'warehouse' }
    }
    case 'unbind_cargo_vehicle': {
      const item = findCargo(store.cargo, args.cargoId || args.orderId)
      await store.unbindCargo(item.id)
      return { message: `货物 ${item.id} 已解除车辆绑定`, targetPage: 'warehouse' }
    }
    case 'update_cargo_status': {
      ...
      return { message: `货物 ${item.id} 已更新为 ${status}`, targetPage: 'tracking' }
    }
    case 'confirm_receipt': {
      const item = findCargo(store.cargo, args.cargoId || args.orderId)
      await store.confirmReceipt(item.id)
      return { message: `货物 ${item.id} 已确认收货`, targetPage: 'tracking' }
    }
    case 'submit_address_change': {
      const item = findCargo(store.cargo, args.cargoId || args.orderId)
      const address = required(args.address || args.detail, '新收货地址')
      ...
      return { message: `货物 ${item.id} 的改址申请已提交`, data: result, targetPage: 'tracking' }
    }
    case 'confirm_unload_address': {
      const item = findCargo(store.cargo, args.cargoId || args.orderId)
      const address = required(args.address, '卸货地址')
      const lat = Number(args.lat)
      const lng = Number(args.lng)
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new Error('卸货地址缺少有效经纬度')
      ...
      return { message: `货物 ${item.id} 的卸货地址已确认`, data: result, targetPage: 'tracking' }
    }
    case 'report_unload_abnormal': {
      ...
      return { message: `货物 ${item.id} 的卸货地址异常已上报`, data: result, targetPage: 'tracking' }
    }
    case 'rate_driver': {
      const item = findCargo(store.cargo, args.cargoId || args.orderId)
      ...
      return { message: `货物 ${item.id} 的司机评价已提交`, data: result, targetPage: 'tracking' }
    }

...
import { chongqingCargoId, chongqingMockCargo, chongqingMockVehicle, chongqingVehiclePlate } from '@/data/chongqingGps'
...
export const useLogisticsStore = defineStore('logistics', () => {
  ...
  let savedArchivedAlerts: DeletedAlertItem[] = []
  try {
    const parsed = JSON.parse(localStorage.getItem('smart-logistics-deleted-alerts') || '[]')
    savedArchivedAlerts = Array.isArray(parsed) ? parsed : []
  } catch {
  ...
  const savedRole = (localStorage.getItem('smart-logistics-role') || sessionStorage.getItem('smart-logistics-role') || 'DISPATCHER') as UserRole
  const savedApiUser = tokenManager.getUser()
  type CurrentUser = Pick<UserProfile, 'id' | 'username' | 'name' | 'role' | 'phone' | 'permissions'> & { roleLabel: string }
  ...
  function ensureChongqingHardwareData() {
    ...
    if (vehicleIndex >= 0) {
      vehicles.value[vehicleIndex] = {
        ...vehicles.value[vehicleIndex],
        ...structuredClone(chongqingMockVehicle),
        image: vehicles.value[vehicleIndex].image,
      }
    } else {
      vehicles.value.unshift(structuredClone(chongqingMockVehicle))
    }

    const cargoIndex = cargo.value.findIndex((item) => item.id === chongqingCargoId)
    if (cargoIndex >= 0) {
      cargo.value[cargoIndex] = {
        ...cargo.value[cargoIndex],
        ...structuredClone(chongqingMockCargo),
      }
    } else {
      cargo.value.unshift(structuredClone(chongqingMockCargo))
    }
  }
  ...
  watch(archivedAlerts, () => {
    localStorage.setItem('smart-logistics-deleted-alerts', JSON.stringify(archivedAlerts.value))
  }, { deep: true })
  ...
  async function login(username: string, password: string, role: UserRole = 'DISPATCHER') {
    ...
    if (usingDemo.value) {
      ...
      if (!account) throw new Error('请输入用户名')
      if (!password) throw new Error('请输入密码')
      if (!Object.prototype.hasOwnProperty.call(demoAccounts, account)) {
        throw new Error('用户名不存在，请检查后重新输入')
      }
      if (demoAccounts[account] !== password) {
      ...
      user.value = { id: 'demo-user', username: account, name: username.trim() || '林若晨', role, phone: '', permissions: [], roleLabel: roleLabels[role] }
      return
    }
    const result = await authApi.login({ username, password })
    setApiUser(result.user)
    // 登录成功后先进入工作台，车辆/告警等数据在后台并行加载，避免被慢接口阻塞。
    await initialize({ waitForData: false, profileLoaded: true })
  }

  async function loginWithFace(imageBase64: string, role: UserRole = 'DISPATCHER') {
    error.value = ''
    if (usingDemo.value) {
      localStorage.setItem('smart-logistics-token', 'demo-token')
      localStorage.setItem('smart-logistics-role', role)
      sessionStorage.removeItem('smart-logistics-token')
      sessionStorage.removeItem('smart-logistics-role')
      user.value = { id: 'demo-user', username: 'face-demo', name: '人脸识别用户', role, phone: '', permissions: [], roleLabel: roleLabels[role] }
      return
    }
    const result = await authApi.faceLogin({
      imageBase64,
      deviceId: getDeviceId(),
      clientTime: new Date().toISOString(),
    })
    setApiUser(result.user)
    // 与密码登录一致：先进入工作台，业务数据在后台并行加载。
    await initialize({ waitForData: false, profileLoaded: true })
  }

  function getDeviceId() {
    const key = 'smart-logistics-device-id'
    const current = localStorage.getItem(key)
    if (current) return current
    const next = crypto.randomUUID?.() || `browser-${Date.now()}-${Math.random().toString(16).slice(2)}`
    localStorage.setItem(key, next)
    return next
  }

  function setApiUser(profile: UserProfile) {
    user.value = { ...profile, roleLabel: roleLabels[profile.role] }
  }

  function applyUserProfile(profile: UserProfile) {
    tokenManager.updateUser(profile)
    setApiUser(profile)
  }
  ...
  async function updateAlert(id: string, status: AlertStatus, resolution = '', remark = '') {
    if (!usingDemo.value) {
      ...
      if (index >= 0) {
        // 告警处理接口只返回状态、时间和备注，不会重复返回完整告警对象。
        // 不能直接用 mapAlert 覆盖，否则 title/severity 等字段会变成 undefined，
        // 模板随后调用 toLowerCase() 时会导致整个告警页渲染失败。
        const current = alerts.value[index]
        const responseStatus = updated.status === 'PENDING' || updated.status === 'ACKNOWLEDGED' || updated.status === 'RESOLVED'
          ? updated.status
          : status
        alerts.value[index] = {
          ...current,
          ...
          logs: updated.logs ?? current.logs,
        }
      }
      return
    }
  ...
  function removeAlerts(ids: string[]) {
    ...
    const removing = alerts.value.filter((item) => targetIds.has(item.id))
    const deletedAt = new Date().toLocaleString('zh-CN')
    if (removing.length) {
      const currentIds = new Set(archivedAlerts.value.map((item) => item.id))
      archivedAlerts.value.unshift(
        ...removing
  ...
  function refreshLiveData() {
    vehicles.value.forEach((item) => {
      const usesPrototypeCoordinate = item.lat >= 0 && item.lat <= 100 && item.lng >= 0 && item.lng <= 100
      if (item.status === 'IN_TRANSIT' && usesPrototypeCoordinate) {
  ...
  async function initialize(options: { waitForData?: boolean; profileLoaded?: boolean } = {}) {
    loading.value = true
    error.value = ''
    try {
      if (!usingDemo.value && tokenManager.getAccessToken()) {
        ...
        const dataLoad = Promise.all([loadVehicles(), loadCargo(), loadAlerts(), loadDevices()])
        startRealtime()
        if (options.waitForData === false) {
          void dataLoad.catch((cause) => {

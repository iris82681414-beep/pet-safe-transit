...
架构设计参考 AZAZ 项目的文档表达方式拆成两张图：系统架构图表达主链路，顶层分层图表达技术选型。两张图都按当前实现口径生成：前端主项目、Spring Boot 后端、MQTT/Kafka 实时链路、PostgreSQL/TimescaleDB/Redis/MinIO 数据层、DeepSeek/百度/高德外部能力都在图中体现。

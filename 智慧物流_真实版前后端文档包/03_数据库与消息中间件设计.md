...
CREATE INDEX idx_active_vehicle_binding ON cargo_vehicle_binding(vehicle_id) WHERE status = 'ACTIVE';
CREATE UNIQUE INDEX ux_active_cargo_binding ON cargo_vehicle_binding(cargo_id) WHERE status = 'ACTIVE';
```

绑定规则：

- 一辆车可以同时绑定多个 `ACTIVE` 货物，用来支持一车多货运输。
- 一个货物同一时间只能绑定一辆车，因此 `cargo_id` 的 `ACTIVE` 绑定仍然保持唯一。

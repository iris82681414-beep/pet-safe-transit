...
async function passwordLogin() {
  ...
  try {
    ...
    phase.value = 'idle'
    emit('transitionEnd')
    ElMessage.error(error instanceof Error ? error.message : '登录失败，请检查账号或服务配置')
  } finally {
    loading.value = false
  }
}
...
async function startCamera() {
  try {
    ...
    ElMessage.warning('未获得摄像头权限，请允许访问后重试')
  }
}
...
async function faceLogin() {
  ...
  const image = await new Promise<Blob | null>((resolve) => output.toBlob(resolve, 'image/jpeg', .88))
  if (!image) return
  ...
  emit('transitionStart')
  try {
    await store.loginWithFace(image, form.role)
    stopCamera()
    await playEntrySequence()
  } catch (error) {
    phase.value = 'idle'
    emit('transitionEnd')
    ElMessage.error(error instanceof Error ? error.message : '人脸识别失败，请改用账号密码登录')
  } finally {
    loading.value = false
  }
}

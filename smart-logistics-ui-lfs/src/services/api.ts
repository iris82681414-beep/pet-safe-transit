...
export const authApi = {
  ...
  async faceLogin(image: Blob): Promise<LoginResponse> {
    const formData = new FormData()
    formData.append('face', image, 'face-login.jpg')
    const result = await apiClient.upload<LoginResponse>('/auth/face-login', formData, { skipAuth: true, skipTokenRefresh: true })
    tokenManager.save(result, result.user)
    return result
  },
